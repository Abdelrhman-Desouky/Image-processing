function [ new_img ] = transformation( old_img,index,c,y)
%index.. 1=log  /  2=invert  / 3= power  /  4=root  / 5=negative .
[h w l]=size(old_img);
trans_rgb=zeros(h,w);
...
for i=1:h
    for j=1:w
        ...
        if index==2
            if l==3
                trans_rgb(i,j,1)=exp(old_img(i,j,1));
                trans_rgb(i,j,2)=exp(old_img(i,j,2));
                trans_rgb(i,j,3)=exp(old_img(i,j,3));
            else
                trans_gray(i,j)=exp(old_img(i,j));
            end
        end
        if index==3
             if l==3
